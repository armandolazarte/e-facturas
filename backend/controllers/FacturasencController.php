...
//                         ->andWhere(['impresaproveedor'=>0])
    ...
    public function actionNotificar($id = -1)
    {
        ...
        //    $mensaje = 'Estamos teniendo inconvenientes con la notificaci�n de facturas. El servicio se encuentra temporalmente suspendido.';
...
// 			exit();
            ...
            if (!$QUERY || count($pk_facturas) == 0) {
                ...
                if (count($pk_facturas) > 0) {
                    $mensaje = 'Hubo un error. Repita la b�squeda desde Mis Facturas.';
                }
                return $this->redirect(['error', 'mensaje' => $mensaje]);
            }
