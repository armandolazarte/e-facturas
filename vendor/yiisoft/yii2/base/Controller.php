...
class Controller extends Component implements ViewContextInterface
{
    ...
    /**
     * controla el acceso del usuario ANONIMO
     * Ej: si un usuario no logueado quiere acceder a una url no autorizada
     * es redireccionado a la pagina principal
     * @return url home (pagina principal)
     */
    public function isGuestGoHome() {
    ...
    /**
     * controla el acceso del usuario LOGUEADO
     * Ej: si un usuario logueado quiere acceder a al url login
     * es redireccionado a la pagina principal
     * @return url home (pagina principal)
     */
    public function isUserGoHome() {
    	if (!Yii::$app->user->isGuest) {
    		return $this->goHome();
    	}
    }
    ...
    /**
     * controla que solo acceda el usuario ADMIN
     * Ej: si un usuario anonimo o logueado que no es admin 
     * quiere acceder a sitios de acceso admin
     * es redireccionado a la pagina principal
     * @return url home (pagina principal)
     */
    public function isNotAdminGoHome() {
    	if (Yii::$app->user->isGuest) {
    		return $this->goHome();
    	}
    ...
    public function isNotEmpresaAdminGoHome() {
        if (Yii::$app->user->isGuest) {
            return $this->goHome();
        }
        if (!EmpresasAdmin::isEmpresaAdmin()) {
            return $this->goHome();
        }
    ...
    /**
     * controla que el token recibido no sea nulo
     * Ej: si no se recibe un token
     * es redireccionado a la pagina principal
     * @return url home (pagina principal)
     */
    public function isTokenIsNullGoHome($token) {
    	if ($token == null) {
    		return $this->goHome();
    	}
    }
